...
def get_fc1(last_conv, num_classes, fc_type):
    ...
    if fc_type == 'Z':
        ...
        # Although kernel is not used here when global_pool=True,
        # we should put one
...
def residual_unit_v3(data, num_filter, stride, dim_match, name, **kwargs):
    ...
    conv1 = Conv(data=bn1, num_filter=num_filter, kernel=(3, 3), stride=(1, 1),
                 pad=(1, 1),
...
def get_head(data, version_input, num_filter):
    ...
    data = data * 0.0078125
    if version_input == 0:
        body = Conv(data=data, num_filter=num_filter, kernel=(7, 7),
                    stride=(2, 2), pad=(3, 3),
        ...
        body = Conv(data=body, num_filter=_num_filter, kernel=(3, 3),
                    stride=(1, 1), pad=(1, 1),
                    no_bias=True, name="conv0", workspace=workspace)
        body = mx.sym.BatchNorm(data=body, fix_gamma=False, eps=2e-5,
                                momentum=bn_mom, name='bn0')
        body = Act(data=body, act_type='relu', name='relu0')
        body = residual_unit_v3(body, _num_filter, (2, 2), False, name='head',
                                **kwargs)
    return body
