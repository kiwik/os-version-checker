...
This is going to do the samem as 'python run_squad.py --do_predict=True ...'
using a squad/bert model that was converted to onnx. Lots of code was taken
from run_squad.py.
You run it with:
...
Feature = collections.namedtuple("Feature", ["unique_id", "tokens",
                                             "example_index",
                                             "token_to_orig_map",
...
def convert_examples_to_features(examples, tokenizer, max_seq_length,
    ...
    for (example_index, example) in enumerate(examples):
        ...
        for (doc_span_index, doc_span) in enumerate(doc_spans):
            ...
            unique_id += 1
    return (np.array(res_input_ids), np.array(res_input_mask),
            np.array(res_segment_ids), extra)
        ...
        for (feature_index, feature) in enumerate(features):
            ...
            for start_index in start_indexes:
                for end_index in end_indexes:
                    ...
                    # predict that the start of the span is in the question.
...
def main():
    ...
    parser.add_argument('--max_seq_length', type=int,
                        default=256, help='max_seq_length')
    parser.add_argument('--max_query_length', type=int,
                        default=64, help='max_query_length')
    parser.add_argument('--max_answer_length', type=int,
                        default=30, help='max_answer_length')
    parser.add_argument('--n_best_size', type=int,
                        default=20, help='n_best_size')
    parser.add_argument('--doc_stride', type=int,
                        default=128, help='doc_stride')
    ...
    for idx in range(0, n, bs):
        ...
        for i in range(0, in_batch):
            ...
            all_results.append(RawResult(
                unique_id=unique_id, start_logits=result[0][0][i],
